...
WORD c_clamp(int x) {
    // // Clamp the RGB values to 0-255 range
    WORD r = (x < 16) ? 16 : (x > 255) ? 240 : (WORD)x;
...
int main() {
    ...
    //#pragma clang loop interleave(enable) vectorize_width(fixed)
    //vectorize_width(scalable) interleave(enable) #pragma clang loop interleave(enable)
    //#pragma clang loop interleave(enable) vectorize_width(fixed)
    ...
    #pragma clang loop vectorize(assume_safety) vectorize_width(fixed)
    for (i = 0; i < bitmapInfoHeader.biHeight - 2; i += 2) {
        ...
        for (j = 0; j < bitmapInfoHeader.biWidth - 2; j += 2) {
            index = (i * bitmapInfoHeader.biWidth + j) * (3);

            index2 = ((i + 1) * bitmapInfoHeader.biWidth + j) * (3);
    ...
    //vectorize_width(scalable) interleave(enable)
    #pragma omp parallel for simd

...
// void *rgbToYCCThread(void *arg) {
//     // Calculate the range of rows this thread will handle
//     int start_row = *(int *)arg;
//     int end_row = start_row + (bitmapInfoHeader.biHeight / NUM_THREADS);

//     for (int i = start_row; i < end_row - 2; i += 2) {
//         for (int j = 0; j < bitmapInfoHeader.biWidth - 2; j += 2) {
//             // RGB to YCC conversion logic

//             // Update outputData, imageData, or new_output_data

//             // Update outputIndex and outputIndex2 accordingly
//         }
//     }
...
int main() {
    ...
    // // Initialize Pthreads and create threads
    // pthread_t threads[NUM_THREADS];
    // int thread_args[NUM_THREADS];

    // for (int i = 0; i < NUM_THREADS; i++) {
    //     thread_args[i] = i * (bitmapInfoHeader.biHeight / NUM_THREADS);
    //     pthread_create(&threads[i], NULL, rgbToYCCThread, &thread_args[i]);
    // }

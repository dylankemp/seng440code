...
void rgbToCmyk(BYTE redtl, BYTE greentl, BYTE bluetl, BYTE redtr, BYTE greentr, BYTE bluetr, BYTE redbl, BYTE greenbl, BYTE bluebl, BYTE redbr, BYTE greenbr, BYTE bluebr, unsigned char *Ytl,  unsigned char *Ytr,  unsigned char *Ybl,  unsigned char *Ybr, unsigned char *CB, unsigned char *CR) {
    ...
    float y_tl = (float)(16.0 + 0.257 *  Rtl + 0.504 *  Gtl + 0.098 *  Btl);
    float y_tr = (float)(16.0 + 0.257 *  Rtr + 0.504 *  Gtr + 0.098 *  Btr);
    float y_bl = (float)(16.0 + 0.257 *  Rbl + 0.504 *  Gbl + 0.098 *  Bbl);
    float y_br = (float)(16.0 + 0.257 *  Rbr + 0.504 *  Gbr + 0.098 *  Bbr);

    float cb_tl = (float)(128 - 0.148 *  Rtl - 0.291 *  Gtl + 0.439 *  Btl);
    float cr_tl = (float)(128 + 0.439 *  Rtl - 0.368 *  Gtl - 0.071 *  Btl);

    float cb_tr = (float)(128 - 0.148 *  Rtr - 0.291 *  Gtr + 0.439 *  Btr);
    float cr_tr = (float)(128 + 0.439 *  Rtr - 0.368 *  Gtr - 0.071 *  Btr);

    float cb_bl = (float)(128 - 0.148 *  Rbl - 0.291 *  Gbl + 0.439 *  Bbl);
    float cr_bl = (float)(128 + 0.439 *  Rbl - 0.368 *  Gbl - 0.071 *  Bbl);

    float cb_br = (float)(128 - 0.148 *  Rbr - 0.291 *  Gbr + 0.439 *  Bbr);
    float cr_br = (float)(128 + 0.439 *  Rbr - 0.368 *  Gbr - 0.071 *  Bbr);
...
int main() {
    ...
    for (i = 0; i < bitmapInfoHeader.biHeight -2 ; i += 2) {
        for (j = 0; j < bitmapInfoHeader.biWidth -1; j += 2) {
            ...
            // printf("2;\n");
            rgbToCmyk(redtl, greentl, bluetl, redtr, greentr, bluetr, redbl, greenbl, bluebl, redbr, greenbr, bluebr, &outputData[index], &outputData[outputIndex + 1], &outputData[outputIndex + 2],&outputData[outputIndex + 3],&outputData[outputIndex + 4],&outputData[outputIndex + 5]);
            // printf("3;\n");
    ...
    new_output_data = (unsigned char*)malloc(sizeof(BYTE)*height*width*3);
